...
- **LilypadDbCache** is a `LilypadCache` backed by one table described by a `LilypadDbSchema<V>`.
  ...
  - **Schema check** (`src/dbGate/LilypadSchemaCheck.ts`): `checkLilypadSchema` reads the catalogs only. The cache runs it once per instance (`sync.verify`): `warn` before LISTEN (it resolves the schema used to filter notifications) or in the background with the first changelog read; `throw` in `create`. With `warn`, a check that could not run (an error, not problems found) is forgotten, and a later read runs it again in the background after a backoff (`checkSchemaInBackground`). The `listen` check requires the notifying row triggers to cover INSERT, UPDATE and DELETE together (possibly split across triggers). The unit tests mock it, as they mock `readLilypadChanges`.
