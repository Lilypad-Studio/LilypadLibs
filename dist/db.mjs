...
var LilypadDbGate = class _LilypadDbGate {
  ...
  heartbeatBackoff = new LilypadBackoff(() => 1e3);
  closing;
  constructor(options) {
...
var LilypadChangelogReader = class {
  ...
  async readAll(subscribers) {
    ...
    await Promise.allSettled(
      subscribers.map(
        async (subscriber, index) => subscriber.apply({ changes: changes[index] ?? [], cursor, readAt }, requests[index])
      )
    );
  }
};
