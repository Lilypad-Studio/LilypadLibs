...
export interface LilypadLoggerConstructorOptions<T extends string> {
    ...
    errorLogging?: (error: unknown) => void;
}
type ChannelMethods<T extends string> = {
    [K in T]: (message: unknown) => void;
};
/**
 ...
 * ```
 */
export default function createLogger<T extends string = 'log' | 'error' | 'warn'>(options: LilypadLoggerConstructorOptions<T>): LilypadLogger<T> & ChannelMethods<T>;
export {};
