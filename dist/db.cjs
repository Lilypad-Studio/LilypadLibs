...
var LilypadDbGate = class LilypadDbGate {
	constructor(options) {
		...
		if (listenerConnectionString && listenerConnectionString !== options.connectionString) this.listenerClient = (0, postgres.default)(listenerConnectionString);
		if (options.listenHeartbeat !== false) this.heartbeat = new LilypadListenHeartbeat(options.listenHeartbeat ?? DEFAULT_LISTEN_HEARTBEAT, () => this.sql`SELECT pg_notify(${this.heartbeatChannel}, '')`, (error) => require_LilypadCacheCore.libLog(this.logger, "debug", this.id, "LISTEN heartbeat failed:", error));
	}
	/**
	...
	*/
	static async create(options) {
		return require_LilypadSingleton.createLilypadSingletonAbleAsync("LilypadDbGate", options, async (release) => {
			const instance = await LilypadDbGate.initializeNew(options);
			instance.releaseSingleton = release;
			return instance;
		}, {
			value: (0, node_crypto.createHash)("sha256").update(JSON.stringify([
			...
			onMismatch: () => require_LilypadCacheCore.libLog(options.logger, "warn", "LilypadDbGate", `Singleton "${options.singleton ? options.singletonIdentifier : ""}" already exists with different connection options: the new options are ignored.`)
		});
	}
	...
	*/
	initializeListener(channel) {
		require_LilypadCacheCore.libLog(this.logger, "debug", this.id, `Initializing listener for channel "${channel}".`);
		const listener = {
	...
	*/
	runCallbackSafely(channel, callbackId, callback) {
		Promise.resolve().then(callback).catch((error) => {
			require_LilypadCacheCore.libLog(this.logger, "error", this.id, `Error in listener callback "${callbackId}" for channel "${channel}":`, error);
		});
	}
	...
	executeReconnectCallbacks(channel, listener) {
		require_LilypadCacheCore.libLog(this.logger, "warn", this.id, `LISTEN on channel "${channel}" was re-established: notifications sent meanwhile are lost.`);
	...
	*/
	async addListener(identifier) {
		...
		require_LilypadCacheCore.libLog(this.logger, "debug", this.id, `Adding listener for channel "${channel}" with callback ID "${callbackId}".`);
		...
		require_LilypadCacheCore.libLog(this.logger, "debug", this.id, `Listener for channel "${channel}" has ${listener.callbacks.size} callbacks.`);
	}
	/**
	...
	*/
	async removeListener(channel, callbackId) {
		...
		if (listener.callbacks.size === 0) {
			...
			try {
				...
				require_LilypadCacheCore.libLog(this.logger, "warn", this.id, `Could not stop listening on "${channel}":`, error);
			}
		}
		return true;
	}
	...
	async startHeartbeat() {
		...
		try {
			...
			require_LilypadCacheCore.libLog(this.logger, "warn", this.id, "Could not start the LISTEN heartbeat:", error);
		}
	}
	async stopHeartbeat() {
...
var LilypadChangelogSync = class {
	...
	beforeRead() {
		...
		if (this.options.poll === "background") {
			require_LilypadPlatform.runInBackground(this.host.platform, reading, () => {});
			return;
		}
		return reading;
	}
...
var LilypadSchemaVerifier = class {
	...
	*/
	checkInBackground(now = Date.now()) {
		...
		require_LilypadPlatform.runInBackground(this.options.platform, this.verify(), () => {});
	}
...
*/
var LilypadDbCache = class LilypadDbCache extends require_LilypadCacheCore.LilypadCacheCore {
	/**
	...
	*/
	static async create(options) {
		return require_LilypadSingleton.createLilypadSingletonAbleAsync("LilypadDbCache", options, async (release) => {
			...
			onMismatch: () => require_LilypadCacheCore.libLog(options.logger, "warn", "LilypadDbCache", `Singleton "${options.singleton ? options.singletonIdentifier : ""}" already exists with a different table or TTL: the new options are ignored.`)
		});
	}
	constructor(options) {
		...
		this.verifier = new LilypadSchemaVerifier({
			...
			log: (level, ...message) => require_LilypadCacheCore.libLog(this.logger, level, this.name, ...message),
		...
		require_LilypadCacheCore.libLog(this.logger, "debug", this.name, `LilypadDbCache initialized for table "${schema.tableName}" (sync: ${sync.strategy})`);
	}
	...
	loadTable() {
		if (!this.tableLoad) {
			const loading = this.bulkSyncFlowControl.executeWithTimeout((signal) => this.loadRows(signal)).catch((error) => {
				require_LilypadCacheCore.libLog(this.logger, "error", this.name, "Error loading the table: ", error);
				throw error;
			}).finally(() => {
	...
	*/
	async queryRows(keys, shared = false) {
		...
		try {
			...
			require_LilypadCacheCore.libLog(this.logger, "error", this.name, "Error fetching rows of the table: ", error);
			throw error;
		}
	}
	/**
	...
	*/
	async sqlCreate(item) {
		...
		if (key === void 0) {
			require_LilypadCacheCore.libLog(this.logger, "warn", this.name, `The row created in "${this.schema.tableName}" has no primary key "${String(this.schema.primaryKey)}" after selectSanitizationFn: it is not cached.`);
			return row;
		}
...
exports.lilypadChangelogSql = lilypadChangelogSql;
exports.lilypadChangelogTriggerSql = lilypadChangelogTriggerSql;
exports.lilypadServerlessPool = lilypadServerlessPool;
exports.pruneLilypadChangelog = pruneLilypadChangelog;
exports.readLilypadChanges = readLilypadChanges;
