import { i as toTtlSeconds, n as runInBackground, r as sharedStoreOperation, t as runAfterResponse } from "./LilypadPlatform-Cdm5WJuh.mjs";
import { i as assertNumberOption, t as LilypadFlowControl } from "./LilypadFlowControl-bLx7gUhG.mjs";
...
var LilypadSharedLevel = class {
	...
	operation(description, operation, fallback) {
		...
		return sharedStoreOperation(() => operation(store), fallback, timeout, (error) => warn(`Shared cache ${description} failed:`, error));
	}
	inBackground(description, operation) {
		runInBackground(this.options.platform, this.operation(description, operation, void 0), () => {});
	}
	/**
	...
	*/
	...
	async acquireLock(normalizedKey) {
		...
		await this.operation(`write of the lock of "${normalizedKey}"`, (store) => store.set(this.lockKey(normalizedKey), owner, { ttl: toTtlSeconds(lockTtl) }), void 0);
		return owner;
	}
...
var LilypadCacheCore = class {
	constructor(options = {}) {
		...
		this.flowControl = new LilypadFlowControl({ timeout: options.fetchTimeout ?? DEFAULT_FETCH_TIMEOUT });
		this.bulkSyncFlowControl = new LilypadFlowControl({ timeout: options.bulkSync?.timeout ?? DEFAULT_BULK_SYNC_TIMEOUT });
	...
	*/
	refreshInBackground(key, valueFn, options, lockedByOtherInstance) {
		...
		runAfterResponse(this.platform, async () => {
	...
	*/
	emitInvalidation(source, keys, options = {}) {
		...
		runInBackground(this.platform, Promise.resolve().then(() => onInvalidate(event)), (error) => libLog(this.logger, "error", this.name, "Error in onInvalidate:", error));
	}
	/**
...
//#endregion
export { libLog as n, LilypadCacheCooldownError as r, LilypadCacheCore as t };

//# sourceMappingURL=LilypadCacheCore-CwziKox7.mjs.map